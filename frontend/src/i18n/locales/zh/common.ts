...
userManual: {
    ...
    quickStart: {
      ...
      step3Title: "第三步：获取 API Key 并配置",
        step3Desc: "登录本平台后，点击左侧菜单「API 密钥」→「创建密钥」。拿到 Key 之后，点击操作栏里的「使用密钥」按钮，系统会一步步引导你完成配置，不需要手动填地址。",
      ...
      codex:step1Title: "第一步：获取 API Key",
        ...
        step3Title: "第三步：安装 Codex",
        step3Desc: "双击刚才下载的安装文件 → 一路点「下一步」或「继续」→ 直到提示「安装完成」。整个过程不需要做任何选择，全部默认即可。1-2 分钟搞定。",
        step4Title: "第四步：打开 Codex 并配置",
        step4Desc: "双击桌面上的 Codex 图标打开软件。找到软件里的「设置」（一般是右上角的齿轮图标）→ 找到「API 配置」或「自定义接口」→ 看到两个输入框：",
        step5Title: "第五步：填写 API 地址",
        step5Desc: "在第一个输入框（Base URL / API 地址）里粘贴：https://api.lxjfly.com",
        step6Title: "第六步：填写 API Key",
        step6Desc: "在第二个输入框（API Key / 密钥）里粘贴第一步复制的密钥。",
        step7Title: "第七步：保存并开始使用",
        step7Desc: "点击「保存」或「确认」按钮。回到 Codex 主界面，现在就可以直接跟它对话了——在输入框里用中文告诉它你想做什么，它会自动帮你完成。",
    ...
    claude: {
      ...
      step1Desc: '访问 Anthropic 官方文档，按指引安装 Claude Code CLI 工具。',
      step2Title: '获取 API Key',
      step2Desc: '在本平台生成一个 API Key，用于 Claude Code 的身份认证。',
      step3Title: '配置自定义 API 地址',
      step3Desc: '设置环境变量，将 Claude Code 的 API 请求指向本平台：',
    },
    gemini: {
      step1Title: '安装 Gemini CLI',
      step1Desc: '访问 Google Gemini CLI 官方页面，下载对应操作系统的 CLI 工具。',
      step2Title: '获取 API Key',
      step2Desc: '在本平台生成一个 API Key，作为 Gemini CLI 的认证凭证。',
      step3Title: '设置 API Key',
      step3Desc: '将你的 API Key 设置为环境变量：',
      step4Title: '配置自定义 API 地址',
      step4Desc: '设置自定义 API 地址环境变量：',
    },
    chatbox: {
      step1Title: '下载 ChatBox',
      step1Desc: '访问 chatboxai.app 下载对应操作系统的桌面客户端或使用 Web 版。',
      step2Title: '添加自定义 Provider',
      step2Desc: '打开 ChatBox 设置 → AI Provider → 添加自定义 Provider。',
      step3Title: '填写 API 地址',
      step3Desc: 'API 地址填写 https://api.lxjfly.com，API Key 填写你在本平台生成的密钥。',
      step4Title: '选择模型开始对话',
      step4Desc: '保存后在对话界面选择你刚添加的 Provider，即可开始使用。',
    },
  },
}